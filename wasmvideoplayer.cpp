...
#define MIN(X, Y)  ((X) < (Y) ? (X) : (Y))
    ...
    void simpleLog(const char* format, ...) {
        ...
        int tmYear = now_time->tm_year + 1900;
        int tmMon = now_time->tm_mon + 1;
        int tmMday = now_time->tm_mday;
        int tmHour = now_time->tm_hour;
        int tmMin = now_time->tm_min;
        int tmSec = now_time->tm_sec;
        sprintf(szTime, "%d-%d-%d %d:%d:%d", tmYear, tmMon, tmMday, tmHour, tmMin, tmSec);
#endif // __linux
    ...
    ErrorCode processDecodedVideoFrame(AVFrame* frame,int cost) {
        ErrorCode ret = kErrorCode_Success;
        ...
        do {
            ...
            */
            ...
            decoder->videoCallback(decoder->yuvBuffer, decoder->videoSize, timestamp, cost);
        } while (0);
        return ret;
    }
        ...
        *decodedLen = 0;
        ...
        while (ret >= 0) {
            ...
            if (ret == AVERROR(EAGAIN)) {
                ...
                clock_t end = clock();
#ifdef _WIN64
                int cost = (end - decoder->begin) ;
#else
                int cost = (end - decoder->begin) / 1000;
#endif // __linux

                int r = isVideo ? processDecodedVideoFrame(decoder->avFrame, cost) : processDecodedAudioFrame(decoder->avFrame);
    ...
    ErrorCode initDecoder(int fileSize, int logLv) {
        simpleLog("Decoder initialized begin  fileSize %d logLv:%d ", fileSize, logLv);
        ErrorCode ret = kErrorCode_Success;
        do {
            //Log level.
            ...
            if (fileSize >= 0) {
                ...
                sprintf(decoder->fileName, "tmp-%lu.mp4", 1);
                ...
                simpleLog("Decoder initialized  init fifo  %d", kDefaultFifoSize);
            }
        } while (0);
        simpleLog("Decoder initialized  end %d.", ret);
        return ret;
    }
    ...
    ErrorCode openDecoder(int* paramArray, int paramCount, VideoCallback videoCallback, long audioCallback, long requestCallback) {
        ...
        int params[7] = { 0 };
        do {
            simpleLog("Opening decoder begin.");
    ...
    ErrorCode decodeOnePacket() {
        ...
        do {
            if (decoder == NULL) {
                simpleLog("decodeOnePacket  decoder null .");
                ret = kErrorCode_Invalid_State;
                break;
            }

            if (getAailableDataSize() <= 0) {
                simpleLog("decodeOnePacket  getAailableDataSize  null ");
                ret = kErrorCode_Invalid_State;
                break;
            }
            ...
            do {
                decoder->begin = clock();
                ret = (ErrorCode)decodePacket(&packet, &decodedLen);
                if (ret != kErrorCode_Success) {
                    break;
                }
    ...
    void vcb(unsigned char* buff, int size, double timestamp,int cost) {
        printf("-------size:%d  %lf %d ms \n", size, timestamp, cost);
    }

    int main() {
        //return 0;
        int res = initDecoder(-1, 1);
        if (res) {
            printf("initDecoder:%d\n", res);
            return 0;
        }

        FILE* file = fopen("d:/15s.h265", "rb+");
        const int FILE_TRUNK = 4096;
        if (file != NULL) {
            uint8_t data[FILE_TRUNK];
            while (!feof(file)) {
                int size = fread(data, 1, FILE_TRUNK, file);
                sendData(data, size);
            }
        }

        int para[10];
        res = openDecoder(para, 10, vcb, 0, 0);
        if (res) {
            printf("openDecoder:%d\n", res);
            return 0;
        }

        while (1) {
            int res = decodeOnePacket();
            if (res == kErrorCode_Invalid_State) {
                break;
            }
#ifdef _WIN64
            Sleep(40);
